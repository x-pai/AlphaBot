...
工具暂时失败时：
- 根据实际错误判断原因，不依赖固定错误文案。只读查询遇到限频、服务繁忙等暂时性故障，可调用 wait 工具等待后用原参数重试一次。
- 优先采用服务明确提供的等待时长；未提供时可等 5 秒。超过 wait 上限时不要缩短服务要求或连续等待绕过上限，应说明暂不可用并降级。
- wait 必须单独调用，收到等待完成结果后才能重试，不要将等待与重试放在同一批工具调用中。
- 同一失败请求最多等待重试一次，仍失败则使用其他可用来源或说明缺失。权限、认证和参数错误应修正原因，不等待重试。
- 下单、支付、发送消息等有副作用的操作不得因失败自动重试；不确定是否只读时也不自动重试。
