...
async def get_worldcup_overview(_: None = Depends(check_usage_limit)):
    overview = await WorldCupService.get_overview()
...
async def list_worldcup_matches(
    ...
    matches = await WorldCupService.list_matches(stage=stage, status=status)
    return api_response(data=matches)


@router.get("/matches/{match_id}", response_model=dict)
async def get_worldcup_match(match_id: str, _: None = Depends(check_usage_limit)):
    match = await WorldCupService.get_match_detail(match_id)
    if not match:
