...
- **市场主页**：可设置 `DEFAULT_MARKET_DATA_SOURCE=tdxaidata` 全量切换通达信数据，安装 `backend/requirements.txt` 并在本地私密配置提供 `TDXAIDATA_TOKEN`。不自动回退 XGB/THS。接口口径、缺失能力、Docker 部署和验收方法见 [接入说明](../backend/docs/TDXAIDATA.md)。默认 `xgb` 模式继续使用原 XGB/THS 配置。
